...
import { BlockStack, Card, Text, InlineStack, ButtonGroup, Button, ProgressBar, Box, Collapsible, Tooltip, Spinner, Icon, Popover, ActionList, Image } from '@shopify/polaris';
import { MenuHorizontalIcon, ChevronDownIcon, ChevronUpIcon, CheckIcon, XIcon } from '@shopify/polaris-icons';

// ✅ Removed: import styles from './SetupGuide.module.css';

export const SetupGuide = ({ onDismiss, onStepComplete, items }) => {
    const firstIncompleteId = items.find((i) => !i.complete)?.id ?? null;
    ...
    const isAllCompleted = completedItemsLength === items.length;

    // ✅ Always expand ONLY an incomplete item
    useEffect(() => {
        // if nothing incomplete, collapse all
        if (firstIncompleteId === null) {
            setExpanded(null);
            return;
        }

        // if current expanded is completed / invalid, jump to first incomplete
        const expandedItem = items.find((i) => i.id === expanded);
        if (!expandedItem || expandedItem.complete) {
            setExpanded(firstIncompleteId);
        }
    }, [items, firstIncompleteId, expanded]);
                    ...
                    <InlineStack align="space-between" blockAlign="center">
                        ...
                        <ButtonGroup gap="tight" noWrap>
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    setIsGuideOpen((prev) => {
                                        if (!prev) setExpanded(firstIncompleteId); // ✅ only incomplete
                                        return !prev;
                                    });
                                }}
                        ...
                        {items.map((item) => {
                            return (
                                <SetupItem
                                    ...
                                    setExpanded={() => {
                                        if (!item.complete) setExpanded(item.id);
                                    }}
...
const SetupItem = ({
    ...
    // ✅ Inline styles replacing CSS module
    const setupItemStyle = {
        padding: '0.25rem 0.5rem',
        borderRadius: '0.5rem',
        backgroundColor: expanded ? 'inherit' : undefined
    };
    ...
    const contentColStyle = {
        flex: '1 1 auto',
        minWidth: 0, // allows text wrap instead of breaking layout
    };
    ...
    // ✅ same behavior as your media queries:
    // hide between 48em..61.871875em and also below 45.625em
    const shouldHideImage =
        typeof window !== 'undefined' &&
        ((window.matchMedia &&
            window.matchMedia('(min-width: 48em) and (max-width: 61.871875em)').matches) ||
            (window.matchMedia && window.matchMedia('(max-width: 45.625em)').matches));

    const [isHovered, setIsHovered] = useState(false);
    const hoverBg = !expanded && isHovered ? '#f7f7f7' : 'transparent';
    const isDisabled = complete;
    return (
        <Box borderRadius="200" background={expanded && 'bg-surface-active'}>
            <div
                style={{
                    ...setupItemStyle,
                    backgroundColor: expanded ? 'inherit' : hoverBg
                }}
            ...
            >
                <InlineStack gap="200" align="start" blockAlign="start" wrap={false}>
                    ...
                    <div
                        style={{
                            ...itemContentStyle,
                            cursor: expanded ? 'default' : isDisabled ? 'not-allowed' : 'pointer',
                            paddingTop: '.15rem'
                        }}
                        onClick={expanded ? () => null : isDisabled ? () => null : setExpanded}
                    >
                        <div style={contentColStyle}>
                            <BlockStack gap="300" id={id} style={{ width: '100%' }}>
                                <Box style={{ width: '100%' }}>
                                            ...
                                            <BlockStack gap="400">
                                                ...
                                                {primaryButton || secondaryButton ? (
                                                    <ButtonGroup gap="loose">
                                                        {primaryButton ? (
                                                            <Button variant="primary" loading={primaryButton.loading} onClick={primaryButton.props.onAction} {...primaryButton.props}>
                                                                {primaryButton.content}
                                                            </Button>
                                                        ) : null}
                                                        {secondaryButton ? (
                                                            <Button variant="secondary" onClick={secondaryButton.props.onAction} {...secondaryButton.props}>
                        ...
                        {image && expanded && !shouldHideImage ? (
                            <div style={imageColStyle}>
                                <Image source={image.url} alt={image.alt} style={{ maxHeight: '7.75rem' }} />
                            </div>
                        ) : null}
                    </div>
                </InlineStack>
